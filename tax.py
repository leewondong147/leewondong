    ...
    try:
        ...
        if "매출" in job_type:
            ...
            if not c_name:
                ...
                c_name = next((c for c in possible_cols if '상호' in str(c) and '대표' not in str(c)), df.columns[6])
        ...
        def format_df(data_list):
            ...
            final_rows = []
            for month, group in display_df.groupby(display_df['작성일자'].apply(lambda x: pd.to_datetime(x).month)):
