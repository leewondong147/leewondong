...
# ==========================================
# 앱 아이콘 및 탭 제목 설정 (Ver 9.8 차단 우회 종결판)
# ==========================================
st.set_page_config(page_title="이원동 이글아이 마스터", page_icon="🦅", layout="wide")
st.title("🦅 이원동의 '이글아이(Eagle Eye)' 최종 마스터 관제탑 (Ver 9.8)")
st.caption("네이버 API 차단 자물쇠를 완벽 우회하여, 9개 늪을 차단하고 진짜 실시간 세력 수급을 정확히 발라냅니다.")

# 1. 🚨 [차단 우회형] 코스피/코스닥 시총 상위 핵심 주도주 200개 명가 사전 구축
def get_clean_market_master():
    # 네이버 차단을 방어하기 위해 대한민국 증시를 이끄는 핵심 200대 기업을 안전핀으로 강제 빌드합니다.
    heavy_stocks = [
...
# 2. 🚨 [차단 우회 특화형] 네이버 실시간 진짜 세력 수급 추출 엔진
def get_naver_real_investors(codes):
    ...
    try:
        # 네이버의 IP 차단을 피하기 위해 40개씩 안전하게 분할 청구합니다.
        ...
        for chunk in chunks:
            ...
            for item in items:
                ...
                # 🛠️ 네이버 주식창과 100% 똑같은 매수/매도 부호 실시간 계산법
                ...
                results[code] = {
                    ...
                    "foreign_direction": f_sign,       # 1: 순매수, -1: 순매도
                    "institution_direction": i_sign,   # 1: 순매수, -1: 순매도
                    "volume": volume
                }
            time.sleep(0.1) # 🚨 네이버 서버가 눈치채지 못하게 0.1초 미세 렉 방어막
    except:
        pass
    return results
