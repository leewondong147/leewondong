...
# ==========================================
# 앱 아이콘 및 탭 제목 설정 (Ver 22.0 장갑판 강화)
# ==========================================
st.set_page_config(page_title="이원동 이글아이 마스터", page_icon="🦅", layout="wide")
st.title("🦅 이원동의 '이글아이(Eagle Eye)' 통합 관제탑 (Ver 22.0)")
st.caption("한국거래소(KRX)의 서버 차단 시 발동되는 백업 명단을 100대 기업으로 대폭 확장하여 생존력을 강화했습니다.")

# 1. 한국거래소(KRX) 시가총액 탑 500 자동 추출 엔진 (차단 방어력 100배 강화)
...
def get_invincible_500_database():
    try:
        ...
        # 🚨 KRX 서버 차단 시 발동되는 무적의 100대 주도주 백업 장갑판
        backup_list = [
