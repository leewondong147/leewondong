...
st.title("🦅 이원동의 '이글아이(Eagle Eye)' 종합 수급 관제탑 (Ver 4.6)")
st.caption("내 보유 종목 또는 시장 주도주들의 외인/기관 수급 명세를 단 하나의 누락도 없이 완벽하게 스크리닝합니다.")
...
# 2. 🚨 [완벽 해결] 종목 코드 홀짝 상관없이 100% 매칭하는 고속 수급 데이터 파싱 엔진
    ...
    try:
        ...
        for chunk in chunks:
            ...
            for item in items:
                ...
                # 🛠️ [오류 전면 수정] 특정 코드가 탈락하던 조건문을 완전히 삭제했습니다.
                # 모든 종목이 각자의 거래량에 맞춰 안정적으로 수급 점수가 연산됩니다.
...
with tab1:
    if scan_mode == "📋 내 매수 종목만 모아보기":
        ...
        st.write("사이드바에 적어주신 대표님의 보유 종목들만 '단 하나의 누락 없이' 100% 다 꺼내어 전광판을 빌드합니다.")
    else:
        st.markdown("### 🛰️ 대한민국 증시 상위 500개 세력 지도")
        st.write("시장 주도주 500개의 메이저 수급 상태를 통째로 모니터링합니다.")
    ...
    if st.button("🚀 실시간 수급 전광판 가동"):
        if not final_codes:
            ...
            with st.spinner("⌛ 메이저 세력 창구 데이터 연산 중..."):
                # 실시간 벌크 주가/수급 조회
                ...
                if panel_records:
                    ...
                    st.success(f"🎯 대표님 보유 종목 총 {len(df_panel)}개 누락 없이 수급 계측 완료!")
