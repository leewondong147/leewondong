...
# ==========================================
# 앱 아이콘 및 탭 제목 설정 (Ver 9.0)
# ==========================================
st.set_page_config(page_title="이원동 이글아이 마스터", page_icon="🦅", layout="wide")
st.title("🦅 이원동의 '이글아이(Eagle Eye)' 최종 마스터 관제탑 (Ver 9.0)")
st.caption("과거의 모든 꼬인 서버 기억을 포맷하고, 순수 500개 전종목 전광판 엔진으로 새로 태어났습니다.")

# 1. 거래소 전체 종목 매퍼 로드 (데이터 타입 완벽 정제)
...
if scan_mode == "🛰️ 시장 상위 500개 전체 스캔":
    scan_count = st.sidebar.slider("📊 스캔할 종목 수", min_value=100, max_value=600, value=500, step=50, key="master_slider")
